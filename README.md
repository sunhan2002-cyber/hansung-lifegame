...
| 담당자 | 역할 | 담당 범위 |
| --- | --- | --- |
| 김선한 | 팀장: 총괄·기획, QA·문서화 | 일정, 요구사항, 품질 검증, 제출물 관리 |
| 김선한 | DB·콘텐츠 데이터 담당 | 사건, 선택지, 결말, 이미지 ID, 저장 상태 구조 |
| 김재겸 | 프론트엔드 개발 담당 | Android 화면, 화면 이동, 사용자 입력 처리 |
| 정원률 | UI/UX 디자인 담당 | 화면 배치, 글씨 크기, 버튼, 이미지 영역, 가독성 |
| 김우영 | 백엔드·게임 로직 담당 | 지표 계산, 선택 반영, 사건 분기, 결말 판정 |
| 신기훈 | 시나리오·콘텐츠 작성 담당 | 사건 문장, 선택지 문장, 결과 문장, 돌발 이벤트 |
...
| 담당자 | 역할 | 이번 주 산출물 |
| --- | --- | --- |
| 김재겸 | 프론트엔드 개발 담당 | Android 프로젝트 뼈대, `MainActivity.kt`, 화면 5개 |
| 정원률 | UI/UX 디자인 담당 | Compose 테마, 공통 UI 컴포넌트 4개 이상 |
| 김우영 | 백엔드·게임 로직 담당 | `GameModels.kt`, `GameEngine.kt`, `GameEngineTest.kt` |
| 신기훈 | 시나리오·콘텐츠 작성 담당 | `events.sample.json` 사건 80개 이상 |
| 김선한 | DB·콘텐츠 데이터 담당 | 결말 20개 이상, 이미지 ID 80개 이상, Repository와 ProgressStore |
| 김선한 | 팀장 | 테스트 30개 이상, 변경 전후 비교표, 8주 범위표 |
